...
def play_animation():
    ...
    if animate== True:
        index +=0.2
        if index >=len(play_walk):index=0
...
def fly_animation():
    ...
    if animate== True:
        fly_index +=0.2
        if fly_index >=len(fly_walk):fly_index=0
...
def obstacle_movement(obstacle_list):
    if obstacle_list:
        for obstacle_rect in obstacle_list:
            obstacle_rect.x -=5
            if obstacle_rect.bottom ==300: screen.blit(fly,obstacle_rect)
            else:
...
while game_Active:
    ...
    for event in pygame.event.get():
     ...
     if event.type == obstacle_timer and game_on==True:
        print('hellooo')
        if randint(0,2):
            obstacle_rect_list.append(fly.get_rect(bottomleft=(randint(800,1100),300)))
            print('flyyyy')
        else:
             obstacle_rect_list.append(enemy.get_rect(bottomleft=(randint(800,1100),530)))   
             print('snailll')
    ...
    #game_stop
        ...
        text_2_sur= text_2.render(f'you managed to run for: {end_game} second.',False, 'black')
        ...
        text_3_sur= text_3.render('Press any key to continue.',False, 'black')
        #display_score=pygame.transform.scale2x(text_2_sur)
